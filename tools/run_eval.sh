...
#
# 输出:
#   work_dirs/eval_out/eval_nuplan/chamfer_<tag>.txt
#   work_dirs/eval_out/eval_nusc/chamfer_<tag>.txt
#   work_dirs/eval_out/vis_nuplan_cam/ 与 work_dirs/eval_out/vis_nusc_cam/（cam 投影，z=-1.6 贴合地面）
#
# 说明:
...
PY=/home/xiaoxuan/miniconda3/envs/maptr/bin/python
export PATH=/home/xiaoxuan/miniconda3/envs/maptr/bin:/usr/local/cuda/bin:$PATH
...
if [ "$SKIP_NUPLAN" = "0" ]; then
  ...
  mkdir -p work_dirs/eval_out/eval_nuplan
  ...
  $PY tools/test_evaluate.py \
    ...
    2>&1 | tee work_dirs/eval_out/eval_nuplan/chamfer_$TAG.txt | grep -E "\| (divider|ped_crossing|boundary|mAP)|NuscMap_chamfer/mAP" | tail -6
fi
...
if [ "$SKIP_NUSC" = "0" ]; then
  ...
  mkdir -p work_dirs/eval_out/eval_nusc
  ...
  $PY tools/test_evaluate.py \
    ...
    2>&1 | tee work_dirs/eval_out/eval_nusc/chamfer_$TAG.txt | grep -E "\| (divider|ped_crossing|boundary|mAP)|NuscMap_chamfer/mAP" | tail -6
fi
...
$PY tools/visualize_nuplan_pred_cam.py \
  MapTRV2/projects/configs/maptrv2/maptrv2_nuplan_full.py \
  "$CKPT" \
  --show-dir work_dirs/eval_out/vis_nuplan_cam \
  --num-samples 4 --score-thresh 0.3 \
  --ann-file data/infos/nuplan_val_eval_sub.pkl 2>&1 | tail -1
$PY tools/visualize_nuplan_pred_cam.py \
  MapTRV2/projects/configs/maptrv2/maptrv2_nusc_official_eval_3cls_align.py \
  "$CKPT" \
  --show-dir work_dirs/eval_out/vis_nusc_cam \
  --num-samples 4 --score-thresh 0.3 \
  --ann-file data/infos/nuscenes_map_infos_eval_sub_orig_path.pkl 2>&1 | tail -1

echo ""
echo "==== 完成！结果: ===="
echo "  nuPlan:  work_dirs/eval_out/eval_nuplan/chamfer_$TAG.txt"
echo "  nuScenes: work_dirs/eval_out/eval_nusc/chamfer_$TAG.txt"
